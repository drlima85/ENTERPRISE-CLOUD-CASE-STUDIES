...
O servidor precisa alcançar os Domain Controllers do ambiente com o perfil para ler os objetos do AD e ter saída de internet para se comunicar com o Azure AD. Sem isso, a instalação nem completa corretamente.

**Conta de serviço com permissão adequada**
O Azure AD Connect precisa de uma conta com permissão suficiente sobre o Active Directory para ler os objetos sincronizados. Vale confirmar essa permissão antes de iniciar a instalação, para não travar no meio do processo através da documentação oficial da microsoft.
A conta que vai ser utilizada para realizar a instalação ou atualização do ADConect, precisa de uma permissão temporária de local de enterprise Admin e uma conta temporária de Global admin no portal Azure. Recomenda-se a utilização do PIM do azure.
...
4. **Colocar o servidor antigo em modo Staging** (ou desativá-lo, dependendo do plano), garantindo que só um servidor esteja ativo exportando por vez, pois ter dois servidores ativos ao mesmo tempo é uma configuração não suportada e pode causar conflito de sincronização.
...
O ambiente passou a operar com o novo Azure AD Connect como servidor ativo, com a sincronização validada previamente em Staging antes de qualquer mudança real ser aplicada, sem nenhuma interrupção perceptível na sincronização entre o Active Directory e o Azure AD durante a transição.
...
Efetuar a troca de modo stagging a cada 90 ou 120 dias já atualizando a versão da compilação, garantindo o modelo de disaster recovery no ambiente.
