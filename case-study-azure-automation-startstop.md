...
Dois agendamentos foram configurados — um para o horário de desligamento (fim do expediente) e outro para o horário de religamento (início do expediente) — cada um disparando o Runbook correspondente automaticamente, sem intervenção manual.

**Identidade com permissão mínima necessária**
O Runbook foi configurado para autenticar usando uma identidade (Managed Identity ou Service Principal) com permissão limitada apenas à ação de start/stop sobre as VMs alvo, seguindo o princípio de menor privilégio em vez de usar uma conta com permissões administrativas amplas.

## Desafios enfrentados

- **Seleção correta das VMs alvo**: garantir que o script atingisse apenas as VMs pretendidas (ambientes não produtivos), evitando desligar por engano uma VM de produção que compartilhasse o mesmo Resource Group.
...
- Processo consistente e auditável — o histórico de execução dos Runbooks no Azure Automation permite confirmar que o desligamento/religamento ocorreu como esperado.
- Modelo replicável para outros grupos de VMs com necessidade semelhante de agendamento.

## Aprendizados

- Automação de start/stop é uma das formas mais simples e efetivas de redução de custo em nuvem — mas o valor real depende de escopo correto (identificar exatamente quais VMs realmente não precisam rodar 24/7).
